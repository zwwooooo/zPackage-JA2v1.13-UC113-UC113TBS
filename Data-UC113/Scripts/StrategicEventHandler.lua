...
function HandleEarlyMorningEvents()
	...
	if ( CheckLastDateSpokenTot(Profil.DEVIN) == 0 ) then
		...
		if ( CheckNPCData1 (Profil.DEVIN) > 3 ) then
			...
			if ( not ( (gWorldSectorX == Devinx) and (gWorldSectorY == Deviny) and (gWorldSectorZ == 0) ) ) then
				...
				if ( p == 1 ) then
					--G9 : UC = G12
				AddNPCtoSector (Profil.DEVIN,12,7,0)
				elseif ( p == 2 ) then
					--D13 : UC = E13
					AddNPCtoSector (Profil.DEVIN,13,5,0)		
				elseif ( p == 3 ) then
					--C5 : UC = C4
					AddNPCtoSector (Profil.DEVIN,4,3,0)					
				elseif ( p == 4 ) then
					--H2 : UC = J6
					AddNPCtoSector (Profil.DEVIN,6,10,0)	
				elseif ( p == 5 ) then
					--C6 : K12
					AddNPCtoSector (Profil.DEVIN,12,11,0)		
				end			
			end
		end
	end
	...
	if CheckMiscFlags2(Profil.CARMEN,Flags2.PROFILE_MISC_FLAG2_DONT_ADD_TO_SECTOR) == 1 then
		...
		-- Move Carmen to A9
		AddNPCtoSector (Profil.CARMEN,9,1,0) 
		...
		if ( CARMENx ~= gWorldSectorX or CARMENy ~= gWorldSectorY ) then
			...
			i = math.random(1, 1)
			if i == 1 then
				AddNPCtoSector (Profil.CARMEN,9,1,0) -- A9
			elseif i == 2 then
